<div>
    <button class="btn btn-primary mb-3" data-bs-toggle="modal" data-bs-target="#createProjetModal">
        <i class="bi bi-plus-lg me-1"></i> {{ __('app.add') }}
    </button>

    <!-- Create/Edit Modal -->
    <div class="modal fade" id="createProjetModal" tabindex="-1" wire:ignore.self>
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button wire:click="saveprojet" class="btn btn-primary">
                        <span wire:loading.remove wire:target="saveprojet">@if($isUpdate) {{ __('app.edit') }} @else {{ __('app.save') }} @endif</span>
                        <span wire:loading wire:target="saveprojet"><span class="spinner-border spinner-border-sm"></span></span>
                    </button>
                </div>
            </div>
        </div>
    ...
    <!-- Delete Modal -->
    <div class="modal fade" id="deleteProjetModal" tabindex="-1" wire:ignore.self>
                ...
                <table class="table table-hover table-striped mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                @if($projet->image)
                                    <img src="{{ asset('assets/img/projet/'.$projet->image) }}" alt="Image" class="rounded" width="50" height="50" style="object-fit:cover;">
                                @else
                                    <span class="badge bg-secondary">{{ __('app.none') }}</span>
                                @endif
                            </td>
                            <td>{{ $projet->title }}</td>
                            <td>{{ Str::limit($projet->description, 80) }}</td>
                            <td>
                                <button wire:click="edit({{ $projet->id }})" class="btn btn-sm btn-outline-primary"><i class="bi bi-pencil"></i></button>
                                <button wire:click="delete({{ $projet->id }})" class="btn btn-sm btn-outline-danger"><i class="bi bi-trash"></i></button>
                            </td>
                        </tr>
                        @empty
