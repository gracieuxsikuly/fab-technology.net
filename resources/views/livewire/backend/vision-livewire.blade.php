<div>
    <button class="btn btn-primary mb-3" data-bs-toggle="modal" data-bs-target="#createVisionModal">
        <i class="bi bi-plus-lg me-1"></i> {{ __('app.add') }}
    </button>

    <!-- Create/Edit Modal -->
    <div class="modal fade" id="createVisionModal" tabindex="-1" wire:ignore.self>
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button wire:click="savevision" class="btn btn-primary">
                        <span wire:loading.remove wire:target="savevision">@if($isUpdate) {{ __('app.edit') }} @else {{ __('app.save') }} @endif</span>
                        <span wire:loading wire:target="savevision"><span class="spinner-border spinner-border-sm"></span></span>
                    </button>
                </div>
            </div>
        </div>
    ...
    <!-- Delete Modal -->
    <div class="modal fade" id="deleteVisionModal" tabindex="-1" wire:ignore.self>
                ...
                <table class="table table-hover table-striped mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                @if($vision->image)
                                    <img src="{{ asset('assets/img/vision/'.$vision->image) }}" alt="Image" class="rounded" width="50" height="50" style="object-fit:cover;">
                                @else
                                    <span class="badge bg-secondary">{{ __('app.none') }}</span>
                                @endif
                            </td>
                            <td>{{ $vision->title }}</td>
                            <td>{{ Str::limit($vision->description, 80) }}</td>
                            <td>
                                <button wire:click="edit({{ $vision->id }})" class="btn btn-sm btn-outline-primary"><i class="bi bi-pencil"></i></button>
                                <button wire:click="delete({{ $vision->id }})" class="btn btn-sm btn-outline-danger"><i class="bi bi-trash"></i></button>
                            </td>
                        </tr>
                        @empty
