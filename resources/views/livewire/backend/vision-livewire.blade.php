...
<flux:modal name="create-vision" class="md:w-[800px]"> 
    <div class="space-y-6">
        <div>
            ...
            @endif
            <flux:subheading>Ajouter des details de la vision de l'entreprise</flux:subheading>
        </div>
        ...
        <div class="flex">
            <flux:spacer />
            @if($isUpdate)
            <flux:button type="submit" variant="primary" wire:click='savevion'>Modifier</flux:button>
            @else
            <flux:button type="submit" variant="primary" wire:click='savevion'>Enregistrer</flux:button>
            @endif
        </div>
    </div>
</flux:modal>
...
<flux:modal name="delete-vision" class="min-w-[22rem]">
    <div class="space-y-6">
        <div>
    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400" >
        ...
        <tbody>
            ...
            <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">{{ $vision->id }}</td>
                <td class="px-6 py-4 text-gray-600 dark:text-gray-300">
                    @if($vision->image)
                    <img src="{{ asset('assets/img/vision/'.$vision->image) }}" alt="Image" class="w-16 h-16 object-cover">
                    @else
                    <span class="text-red-500">No Image</span>
                    @endif
                </td>
                <td class="px-6 py-4 text-gray-600 dark:text-gray-300">{{ $vision->title }}</td>
                <td class="px-6 py-4 text-gray-600 dark:text-gray-300">{{ $vision->description }}</td>
                <td class="px-6 py-4">
                    <flux:button wire:click="edit({{ $vision->id }})" size="sm" variant="primary">Edit</flux:button>
                    <br><br>
                    <flux:button wire:click="delete({{ $vision->id }})" size="sm" variant="danger">Delete</flux:button>
                </td>
            </tr>
            @empty
            <tr>
